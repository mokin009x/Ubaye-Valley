...
namespace ProGrids
{
	...
	const int WINDOW_HEIGHT = 260;
        ...
        private const int MAX_LINES = 150; // the maximum amount of lines to display on screen in either direction
        ...
        [SerializeField]
        private pg_ToggleContent gc_SnapToGrid =
            new pg_ToggleContent("Snap", "", "Snaps all selected objects to grid.");

        [SerializeField] private pg_ToggleContent gc_GridEnabled = new pg_ToggleContent("Hide", "Show",
        ...
        [SerializeField] private pg_ToggleContent gc_LockGrid =
            new pg_ToggleContent("Lock", "Unlck", "Lock the perspective grid center in place.");

        [SerializeField] private pg_ToggleContent gc_AngleEnabled = new pg_ToggleContent("> On", "> Off",
        ...
        [SerializeField] private pg_ToggleContent gc_RenderPerspectiveGrid =
            new pg_ToggleContent("Full", "Plane", "Renders a 3d grid in perspective mode.");
        ...
        public void LoadPreferences()
        {
            ...
            if (EditorPrefs.HasKey(pg_Constant.SnapEnabled)) snapEnabled = EditorPrefs.GetBool(pg_Constant.SnapEnabled);
            ...
            renderPlane = EditorPrefs.HasKey(pg_Constant.GridAxis)
            ...
            gridColorX = EditorPrefs.HasKey("gridColorX")
            ...
            gridColorY = EditorPrefs.HasKey("gridColorY")
            ...
            gridColorZ = EditorPrefs.HasKey("gridColorZ")
            ...
            predictiveGrid = EditorPrefs.HasKey(pg_Constant.PredictiveGrid)
        ...
        private readonly GUIStyle gridButtonStyle = new GUIStyle();
        private readonly GUIStyle extendoStyle = new GUIStyle();
        private GUIStyle gridButtonStyleBlank = new GUIStyle();
        private readonly GUIStyle backgroundStyle = new GUIStyle();
        ...
        private Rect r = new Rect(8, MENU_EXTENDED, 42, 16);
        private Rect backgroundRect = new Rect(00, 0, 0, 0);
        private Rect extendoButtonRect = new Rect(0, 0, 0, 0);
        private bool menuOpen = true;
        private float menuStart = MENU_EXTENDED;
        ...
        private Color menuBackgroundColor = new Color(0f, 0f, 0f, .5f);
        private Color extendoNormalColor = new Color(.9f, .9f, .9f, .7f);
        private Color extendoHoverColor = new Color(0f, 1f, .4f, 1f);
...
// On Mac ShowAsDropdown and ShowAuxWindow both throw stack pop exceptions when initialized.
		...
		options.position = new Rect(screenRect.x + r.x + r.width + PAD,
            ...
            if (menuIsOrtho)
            {
                ...
                if (pg_ToggleContent.ToggleButton(r, gc_AngleEnabled, drawAngles, gridButtonStyle,
            ...
            if (pg_ToggleContent.ToggleButton(r, gc_RenderPlaneX, (renderPlane & Axis.X) == Axis.X && !fullGrid,
                gridButtonStyle, EditorStyles.miniButton))
                SetRenderPlane(Axis.X);

            r.y += r.height + PAD;

            if (pg_ToggleContent.ToggleButton(r, gc_RenderPlaneY, (renderPlane & Axis.Y) == Axis.Y && !fullGrid,
                gridButtonStyle, EditorStyles.miniButton))
                SetRenderPlane(Axis.Y);

            r.y += r.height + PAD;

            if (pg_ToggleContent.ToggleButton(r, gc_RenderPlaneZ, (renderPlane & Axis.Z) == Axis.Z && !fullGrid,
                gridButtonStyle, EditorStyles.miniButton))
                SetRenderPlane(Axis.Z);

            r.y += r.height + PAD;

            if (pg_ToggleContent.ToggleButton(r, gc_RenderPerspectiveGrid, fullGrid, gridButtonStyle,
                EditorStyles.miniButton))
            {
                fullGrid = !fullGrid;
                gridRepaint = true;
	...
	pb_Profiler profiler = new pb_Profiler();
        ...
        public void OnSceneGUI(SceneView scnview)
        {
            ...
            if (e.Equals(Event.KeyboardEvent(AXIS_CONSTRAINT_KEY))) toggleAxisConstraint = true;

            if (e.Equals(Event.KeyboardEvent(TEMP_DISABLE_KEY))) toggleTempSnap = true;
            ...
            var camDistance = Vector3.Distance(cam.transform.position, lastPivot); // distance from camera to pivot
            ...
            else
            {
                ...
                if (lockGrid && !cam.InFrustum(pivot) || !lockGrid || scnview != SceneView.lastActiveSceneView)
                {
                    if (plane.Raycast(ray, out dist))
                        ...
                        sceneViewPlanePivot =
                            ray.GetPoint(Mathf.Min(cam.farClipPlane / 2f, planeGridDrawDistance / 2f));
                }

                if (lockGrid)
                {
                    pivot = pg_Enum.InverseAxisMask(sceneViewPlanePivot, renderPlane) +
                            pg_Enum.AxisMask(pivot, renderPlane);
                }
                else
                {
                    ...
                    if (Selection.activeTransform == null || !cam.InFrustum(pivot))
                        pivot = pg_Enum.InverseAxisMask(sceneViewPlanePivot, renderPlane) +
                                pg_Enum.AxisMask(
                                    Selection.activeTransform == null ? pivot : Selection.activeTransform.position,
                                    renderPlane);
                }
            }
		...
		pivotGo.transform.position = pivot;
            ...
            if (drawGrid)
            {
				...
				profiler.LogStart("DrawGridPerspective");
                    ...
                    if (gridRepaint || pivot != lastPivot || Mathf.Abs(camDistance - lastDistance) > lastDistance / 2 ||
                        camDir != prevCamDir)
                    {
                        prevCamDir = camDir;
                        gridRepaint = false;
                        ...
                        if (fullGrid)
                        {
                            ...
                            pg_GridRenderer.DrawGridPerspective(cam, pivot, snapValue,
                                new Color[3] {gridColorX, gridColorY, gridColorZ}, alphaBump);
                        }
                        else
                        {
                            if ((renderPlane & Axis.X) == Axis.X)
                                planeGridDrawDistance = pg_GridRenderer.DrawPlane(cam, pivot + Vector3.right * offset,
                                    Vector3.up, Vector3.forward, snapValue, gridColorX, alphaBump);

                            if ((renderPlane & Axis.Y) == Axis.Y)
                                planeGridDrawDistance = pg_GridRenderer.DrawPlane(cam, pivot + Vector3.up * offset,
                                    Vector3.right, Vector3.forward, snapValue, gridColorY, alphaBump);

                            if ((renderPlane & Axis.Z) == Axis.Z)
                                planeGridDrawDistance = pg_GridRenderer.DrawPlane(cam, pivot + Vector3.forward * offset,
                                    Vector3.up, Vector3.right, snapValue, gridColorZ, alphaBump);
                        }
                    }
#if PROFILE_TIMES
            ...
            if (Selection.activeTransform && pg_Util.SnapIsEnabled(Selection.activeTransform))
            {
                ...
                {
                    var selected = lastTransform;

                    if (!toggleTempSnap)
                    {
                        var old = selected.position;
                        var mask = old - lastPosition;

                        var constraintsOn = toggleAxisConstraint ? !useAxisConstraints : useAxisConstraints;
                        ...
                        if (_snapAsGroup)
                            ...
                            foreach (var t in Selection.transforms)
                                t.position = constraintsOn
                                    ? pg_Util.SnapValue(t.position, mask, snapValue)
                                    : pg_Util.SnapValue(t.position, snapValue);
                    }
                ...
                if (!FuzzyEquals(lastTransform.localScale, lastScale) && _scaleSnapEnabled)
                    if (!toggleTempSnap)
                    {
                        var old = lastTransform.localScale;
                        var mask = old - lastScale;

                        if (predictiveGrid)
                        {
                            var dragAxis = pg_Util.CalcDragAxis(Selection.activeTransform.TransformDirection(mask),
                                scnview.camera);
                            if (dragAxis != Axis.None && dragAxis != renderPlane)
                                SetRenderPlane(dragAxis);
                        }
        ...
        {
            var camAxis = AxisWithVector(Camera.current.transform.TransformDirection(Vector3.forward).normalized);
        ...
        private void DrawGridOrthographic(Camera cam, Axis camAxis, Color primaryColor, Color secondaryColor)
        {
            ...
            var bottomLeft = pg_Util.SnapToFloor(cam.ScreenToWorldPoint(Vector2.zero), snapValue);
            var bottomRight = pg_Util.SnapToFloor(cam.ScreenToWorldPoint(new Vector2(cam.pixelWidth, 0f)), snapValue);
            var topLeft = pg_Util.SnapToFloor(cam.ScreenToWorldPoint(new Vector2(0f, cam.pixelHeight)), snapValue);
            var topRight = pg_Util.SnapToFloor(cam.ScreenToWorldPoint(new Vector2(cam.pixelWidth, cam.pixelHeight)),
                snapValue);

            var axis = VectorWithAxis(camAxis);
            ...
            var bl = cam_right.Sum() > 0
                ? pg_Util.SnapToFloor(bottomLeft, cam_right, _snapVal * PRIMARY_COLOR_INCREMENT)
                : pg_Util.SnapToCeil(bottomLeft, cam_right, _snapVal * PRIMARY_COLOR_INCREMENT);
            ...
            var tl = cam_up.Sum() > 0
                ? pg_Util.SnapToCeil(topLeft, cam_up, _snapVal * PRIMARY_COLOR_INCREMENT)
                : pg_Util.SnapToFloor(topLeft, cam_up, _snapVal * PRIMARY_COLOR_INCREMENT);
            ...
            if (drawAngles)
            {
                var cen = pg_Util.SnapValue((topRight + bottomLeft) / 2f, snapValue);

                var half = width > height ? width : height;

                var opposite = Mathf.Tan(Mathf.Deg2Rad * angleValue) * half;
        ...
        {
            return Mathf.Abs(lhs.x - rhs.x) < .001f && Mathf.Abs(lhs.y - rhs.y) < .001f &&
        ...
        public void SetSnapValue(SnapUnit su, float val, int multiplier)
        {
            ...
            if (EditorPrefs.GetBool(pg_Constant.SyncUnitySnap, true))
            {
                ...
                {
                    var snapInitialized =
                        snapSettings.GetField("s_Initialized", BindingFlags.NonPublic | BindingFlags.Static);
        ...
        {
            return toggleAxisConstraint ? !useAxisConstraints : useAxisConstraints;
        }
        ...
        {
            return toggleTempSnap ? !snapEnabled : snapEnabled;
        }
        ...
        [SerializeField] private static readonly List<Action<float>> pushToGridListeners = new List<Action<float>>();
        [SerializeField] private static readonly List<Action<bool>> toolbarEventSubscribers = new List<Action<bool>>();
