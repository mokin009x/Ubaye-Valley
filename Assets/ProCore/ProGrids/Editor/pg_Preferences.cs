...
namespace ProGrids
{
    public class pg_Preferences
    {
        ...
        private static KeyCode _IncreaseGridSize = KeyCode.Equals;
        private static KeyCode _DecreaseGridSize = KeyCode.Minus;
        private static KeyCode _NudgePerspectiveBackward = KeyCode.LeftBracket;
        private static KeyCode _NudgePerspectiveForward = KeyCode.RightBracket;
        private static KeyCode _NudgePerspectiveReset = KeyCode.Alpha0;
        private static KeyCode _CyclePerspective = KeyCode.Backslash;
        ...
        private static readonly string[] SnapMethod =
        {
            "Snap on Selected Axis",
            "Snap on All Axes"
        };

        private static readonly int[] SnapVals = {1, 0};
        ...
        public static void PreferencesGUI()
        {
            ...
            _alphaBump =
                EditorGUILayout.Slider(
                    new GUIContent("Tenth Line Alpha",
                        "Every 10th line will have it's alpha value bumped by this amount."), _alphaBump, 0f, 1f);
            ...
            _NudgePerspectiveBackward =
                (KeyCode) EditorGUILayout.EnumPopup("Nudge Perspective Backward", _NudgePerspectiveBackward);
            _NudgePerspectiveForward =
                (KeyCode) EditorGUILayout.EnumPopup("Nudge Perspective Forward", _NudgePerspectiveForward);
            _NudgePerspectiveReset =
                (KeyCode) EditorGUILayout.EnumPopup("Nudge Perspective Reset", _NudgePerspectiveReset);
            _CyclePerspective = (KeyCode) EditorGUILayout.EnumPopup("Cycle Perspective", _CyclePerspective);

            if (GUILayout.Button("Reset"))
                if (EditorUtility.DisplayDialog("Delete ProGrids editor preferences?",
                    "Are you sure you want to delete these?, this action cannot be undone.", "Yes", "No"))
                    ResetPrefs();
        ...
        {
            _scaleSnapEnabled =
                EditorPrefs.HasKey("scaleSnapEnabled") ? EditorPrefs.GetBool("scaleSnapEnabled") : false;
            _gridColorX = EditorPrefs.HasKey("gridColorX")
                ? pg_Util.ColorWithString(EditorPrefs.GetString("gridColorX"))
                : GRID_COLOR_X;
            _gridColorY = EditorPrefs.HasKey("gridColorY")
                ? pg_Util.ColorWithString(EditorPrefs.GetString("gridColorY"))
                : GRID_COLOR_Y;
            _gridColorZ = EditorPrefs.HasKey("gridColorZ")
            ...
            _snapMethod = Convert.ToInt32(
                EditorPrefs.HasKey(pg_Constant.UseAxisConstraints)
                    ? EditorPrefs.GetBool(pg_Constant.UseAxisConstraints)
                    : USE_AXIS_CONSTRAINTS
            );
            _BracketIncreaseValue = EditorPrefs.HasKey(pg_Constant.BracketIncreaseValue)
                ? EditorPrefs.GetFloat(pg_Constant.BracketIncreaseValue)
                : .25f;
            _GridUnits = (SnapUnit) (EditorPrefs.HasKey(pg_Constant.GridUnit)
        ...
        public static void SetPreferences()
        {
            ...
            EditorPrefs.SetBool(pg_Constant.UseAxisConstraints, Convert.ToBoolean(_snapMethod));
            ...
            if (pg_Editor.instance != null) pg_Editor.instance.LoadPreferences();
        }
